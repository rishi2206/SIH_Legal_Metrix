...
export default function InspectionDetail() {
  ...
  const handleDownloadReport = (reportId) =>
    runAction(`download-${reportId}`, async () => {
      await api.downloadReport(id, reportId, `inspection-report-${id}.pdf`);
    });
            ...
            {reports.map((r) => (
              ...
              >
                ...
                <Button
                  ...
                  disabled={busy === `download-${r.id}`}
                >
                  {busy === `download-${r.id}` ? <Spinner /> : <Download size={16} />}
                  Download PDF
                </Button>
              </li>
            ))}
